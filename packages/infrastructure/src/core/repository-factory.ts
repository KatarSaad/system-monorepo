...
import { IBaseRepository, IFilterOptions, IPaginationOptions, IPaginatedResult, ISearchOptions, IStatsOptions } from '../interfaces/repository.interface';
import { DatabaseException, NotFoundExceptionInfra } from '../exceptions/infrastructure.exceptions';

export class DynamicRepository<T> implements IBaseRepository<T> {
  constructor(
  ...
  async findById(id: string, options?: IFilterOptions): Promise<T | null> {
    try {
      ...
      throw new DatabaseException(`Failed to find ${this.modelName}`, error);
    }
  }
  ...
  async findMany(options?: IFilterOptions & IPaginationOptions): Promise<IPaginatedResult<T>> {
    try {
      ...
      const [data, total] = await Promise.all([
        this.model.findMany({ ...filterOptions, skip, take: limit }),
  ...
  async update(id: string, data: Partial<T>): Promise<T> {
    try {
      ...
      if ((error as any).code === 'P2025') {
        throw new NotFoundExceptionInfra(this.modelName, id);
      }
  ...
  async search(options: ISearchOptions & IPaginationOptions): Promise<IPaginatedResult<T>> {
    try {
      ...
      const searchConditions = fields.map(field => ({
        [field]: { contains: query, mode: caseSensitive ? 'default' : 'insensitive' }
      }));
  ...
  private buildAggregations(aggregations: Record<string, 'count' | 'sum' | 'avg' | 'min' | 'max'>) {
    ...
    Object.entries(aggregations).forEach(([field, operation]) => {
      if (!result[`_${operation}`]) result[`_${operation}`] = {};
...
export class RepositoryFactory {
  private repositories = new Map<string, DynamicRepository<any>>();

  constructor(private prisma: PrismaClient) {}

  create<T>(modelName: string): IBaseRepository<T> {
    if (!this.repositories.has(modelName)) {
      this.repositories.set(modelName, new DynamicRepository<T>(this.prisma, modelName));
    }
    return this.repositories.get(modelName)!;
  }

  getAvailableModels(): string[] {
    return Object.keys((this.prisma as any)._dmmf.datamodel.models);
  }
}